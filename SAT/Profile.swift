...
class Profile: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.tabBarController!.navigationItem.setRightBarButton(UIBarButtonItem(barButtonSystemItem: .action, target: self, action: #selector(share)), animated: true)
        ...
        let backgroundView = UIView(frame: CGRect(x: 0, y: 0, width: view.frame.size.width, height: view.frame.size.width * 9 / 16))
        scrollView.addSubview(backgroundView)
        
        let profileBackground = UIImageView(frame: CGRect(x: 0, y: 0, width: backgroundView.frame.size.width, height: backgroundView.frame.size.width * 9 / 16))
        profileBackground.image = UIImage(named: "pb")
        backgroundView.addSubview(profileBackground)
    ...
    @objc func editPressed() {
        Global.profilePicture = profileImage.image!
        Global.profileName = username.text!
    ...
    @objc func share() {
        ...
        activityViewController.excludedActivityTypes = [UIActivity.ActivityType.postToFacebook, UIActivity.ActivityType.postToTwitter, UIActivity.ActivityType.message, UIActivity.ActivityType.mail, UIActivity.ActivityType.copyToPasteboard]
